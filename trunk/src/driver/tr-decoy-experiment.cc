...
ostream & operator<<( ostream &s, const Parameters &p )
{
	...
	s << "#   contact map dir: " << p.contact_map_dir << endl;
	s << "#   ln(# of conformations): " << p.log_nconf << endl;
...
void evolutionTest( const Parameters &p, ErrorproneTranslation& fe, Polymerase& poly) {
	...
	//cout << p.protein_length << p.free_energy_cutoff << p.structure_ID << endl;
	CodingDNA g = FolderUtil::getSequenceForStructure(folder, 3*p.protein_length, p.free_energy_cutoff, p.structure_ID);
	pop.init(g, &fe, &poly);
