...
int main( int ac, char **av)
{
	...
	if (p.valid) {
		...
		cout << "\t" << av[0] << " <eval type> <pop size> <ca cost> <error rate> <accuracy weight> <error weight> <structure id> <free energy cutoff> <free energy minimum> <random seed> <gene file name>" << endl;
	}
}
