...
void misfoldDistExperiment(Parameters& p)
{
	...
	cout << p;
	cout << "tr\texpr\trun\tdG\tfitness\tfop\tnu\tnacc\tnrob\tntrunc\tnfold\tfacc\tfrob\tftrunc\tffold\tcfacc\tcfrob\tcftrunc\tcffold\tfdens\tgene" << endl;
	...
	for (; it != runResults.end(); it++) {
		...
		double fitnessDensity = fde.getFitnessDensity(rec.gene, *fe, p.N);
		...
		cout << rec.cost_id << tab << rec.cost << tab << rec.runNumber << tab << dG << tab << fitness << tab << fop << tab << nu << tab
			 ...
			 << fitnessDensity << tab << rec.gene << endl;
