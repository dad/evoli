...
void evolutionTest( const Parameters &p, ErrorproneTranslation& fe) {
	...
	Genotype g = GenotypeUtil::getSequence(folder, p.free_energy_cutoff);
	pair<double,int> fdata = GenotypeUtil::translateAndFold(folder, g);
	fe.changeStructure(fdata.second);
	cout << "# Changed structure to " << fdata.second << endl;
	pop.init(g, &fe, p.u);
		     ...
		     double &ave_fop )
{
	...
	pair<double,int> fdata = GenotypeUtil::translateAndFold(folder, g);
	fe->changeStructure(fdata.second);
	s << "# Changed structure to " << fdata.second << endl;
